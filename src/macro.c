...
{
  return pic_sym_value(translate(pic, sym, senv, assoc_box));
}
...
macroexpand_node(pic_state *pic, pic_value expr, struct pic_senv *senv, pic_value assoc_box)
{
  ...
  case PIC_TT_SYMBOL: {
    return macroexpand_symbol(pic, pic_sym(expr), senv, assoc_box);
  }
  case PIC_TT_PAIR: {
    pic_value car;
    struct pic_macro *mac;
...
er_macro_rename(pic_state *pic)
{
  ...
  return pic_sym_value(translate(pic, sym, mac_env, assoc_box));
}
...
er_macro_compare(pic_state *pic)
{
  ...
  m = translate(pic, pic_sym(a), use_env, assoc_box);
  n = translate(pic, pic_sym(b), use_env, assoc_box);
...
ir_macro_inject(pic_state *pic)
{
  ...
  return pic_sym_value(translate(pic, sym, use_env, assoc_box));
}
...
ir_macro_compare(pic_state *pic)
{
  ...
  m = translate(pic, pic_sym(a), mac_env, assoc_box);
  n = translate(pic, pic_sym(b), mac_env, assoc_box);
...
ir_macro_wrap(pic_state *pic, pic_value expr, struct pic_senv *use_env, pic_value assoc_box, pic_value *ir)
{
  if (pic_sym_p(expr)) {
    pic_value r;
    r = pic_sym_value(translate(pic, pic_sym(expr), use_env, assoc_box));
...
ir_macro_unwrap(pic_state *pic, pic_value expr, struct pic_senv *mac_env, pic_value assoc_box, pic_value *ir)
{
  if (pic_sym_p(expr)) {
    ...
    return pic_sym_value(translate(pic, pic_sym(expr), mac_env, assoc_box));
  }
  else if (pic_pair_p(expr)) {
    return pic_cons(pic,
