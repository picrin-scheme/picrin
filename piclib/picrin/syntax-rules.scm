  ...
  (define (compile ellipsis literals rules)
       ...
       ((many? pat)
        (letrec*
            ((inner-pat
             ...
             (inner-rep
              (generate-representation inner-pat inner-levels inner-selectors))
             (filtered-selectors
              (map (lambda (v) (assq v selectors)) inner-vars))
             ;; ((a . (x1 x2)) (b . (y1 y2 y3))) -> ((x1 x2) (y1 y2 y3))
             (list-of-selectors (map cdr filtered-selectors)))
          #`(map (lambda #,inner-tmps #,inner-rep) #,@list-of-selectors)))
       ((pair? pat)
        #`(cons #,(generate-representation (car pat) levels selectors)
                #,(generate-representation (cdr pat) levels selectors)))))
    ...
    (define (compile-rule pattern template)
      (let ((levels
        ...
        (generate-representation template levels selectors)))
