(define-library (picrin test)
  ...
  (define-syntax test
    (syntax-rules ()
      ((_ test-name expected expr test?)
       (let* ((r (test-runner-current))
         ...
         ((on-test-exit r) r)))
      ((_ expected expr test?)
       (test no-name expected expr test?))
      ((_ expected expr)
       (test no-name expected expr equal?))))
       ...
       (define-syntax name
        (syntax-rules ()
          ((_ test-name expected expr)
           (test test-name expected expr test?))
          ((_ expected expr)
           (name no-name expected expr test?)))))))
