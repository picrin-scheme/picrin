...
"(variable=? (the 'else) (make-identifier (car clause) env)))\n                  (",
"cons the-begin (cdr clause))\n                  (if (and (variable? (cadr clause)",
")\n                           (variable=? (the '=>) (make-identifier (cadr clause",
") env)))\n                      (let ((tmp (make-identifier 'tmp here)))\n        ",
"                (list (the 'let) (list (list tmp (car clause)))\n                ",
"              (list the-if tmp\n                                    (list (car (c",
"ddr clause)) tmp)\n                                    (cons (the 'cond) (cdr cla",
"uses)))))\n                      (list the-if (car clause)\n                      ",
"      (cons the-begin (cdr clause))\n                            (cons (the 'cond",
") (cdr clauses))))))))))\n\n  (define-macro quasiquote\n    (lambda (form env)\n\n   ",
"   (define (quasiquote? form)\n        (and (pair? form)\n             (variable? ",
"(car form))\n             (variable=? (the 'quasiquote) (make-identifier (car for",
"m) env))))\n\n      (define (unquote? form)\n        (and (pair? form)\n            ",
" (variable? (car form))\n             (variable=? (the 'unquote) (make-identifier",
" (car form) env))))\n\n      (define (unquote-splicing? form)\n        (and (pair? ",
"form)\n             (pair? (car form))\n             (variable? (caar form))\n     ",
"        (variable=? (the 'unquote-splicing) (make-identifier (caar form) env))))",
"\n\n      (define (qq depth expr)\n        (cond\n         ;; unquote\n         ((unq",
...
"         (,(the 'let*) (,@(cdr bindings))\n               ,@body))))))\n\n  (define",
"-macro letrec\n    (lambda (form env)\n      `(,(the 'letrec*) ,@(cdr form))))\n\n  ",
"(define-macro letrec*\n    (lambda (form env)\n      (let ((bindings (car (cdr for",
"m)))\n            (body     (cdr (cdr form))))\n        (let ((variables (map (lam",
"bda (v) `(,v #f)) (map car bindings)))\n              (initials  (map (lambda (v)",
" `(,(the 'set!) ,@v)) bindings)))\n          `(,(the 'let) (,@variables)\n        ",
"    ,@initials\n            ,@body)))))\n\n  (define-macro let-values\n    (lambda (",
"form env)\n      `(,(the 'let*-values) ,@(cdr form))))\n\n  (define-macro let*-valu",
"es\n    (lambda (form env)\n      (let ((formal (car (cdr form)))\n            (bod",
"y   (cdr (cdr form))))\n        (if (null? formal)\n            `(,(the 'let) () ,",
"@body)\n            `(,(the 'call-with-values) (,the-lambda () ,@(cdr (car formal",
")))\n              (,(the 'lambda) (,@(car (car formal)))\n               (,(the '",
...
"ine-values\n    (lambda (form env)\n      (let ((formal (car (cdr form)))\n        ",
"    (body   (cdr (cdr form))))\n        (let ((arguments (make-identifier 'argume",
"nts here)))\n          `(,the-begin\n            ,@(let loop ((formal formal))\n   ",
...
"     (,the-lambda\n              ,arguments\n              ,@(let loop ((formal fo",
"rmal) (args arguments))\n                  (if (pair? formal)\n                   ",
"   `((,the-set! ,(car formal) (,(the 'car) ,args)) ,@(loop (cdr formal) `(,(the ",
"'cdr) ,args)))\n                      (if (variable? formal)\n                    ",
"      `((,the-set! ,formal ,args))\n                          '()))))))))))\n\n  (d",
"efine-macro do\n    (lambda (form env)\n      (let ((bindings (car (cdr form)))\n  ",
"          (test     (car (car (cdr (cdr form)))))\n            (cleanup  (car (cd",
"r (cdr form))))\n            (body     (cdr (cdr (cdr form)))))\n        (let ((lo",
"op (make-identifier 'loop here)))\n          `(,(the 'let) ,loop ,(map (lambda (x",
") (list (car x) (car (cdr x)))) bindings)\n            (,the-if ,test\n           ",
"  (,the-begin\n              ,cleanup)\n             (,the-begin\n              ,@b",
"ody\n              (,loop ,@(map (lambda (x) (if (null? (cdr (cdr x))) (car x) (c",
"ar (cddr x)))) bindings)))))))))\n\n  (define-macro when\n    (lambda (form env)\n  ",
"    (let ((test (car (cdr form)))\n            (body (cdr (cdr form))))\n        `",
"(,the-if ,test\n                  (,the-begin ,@body)\n                  #undefine",
"d))))\n\n  (define-macro unless\n    (lambda (form env)\n      (let ((test (car (cdr",
...
"       (let ((clause (car clauses)))\n                     `(,the-if ,(if (and (v",
"ariable? (car clause))\n                                         (variable=? (the",
" 'else) (make-identifier (car clause) env)))\n                                   ",
" #t\n                                    `(,(the 'or) ,@(map (lambda (x) `(,(the ",
"'eqv?) ,the-key (,the-quote ,x))) (car clause))))\n                              ",
" ,(if (and (variable? (car clause))\n                                         (va",
"riable=? (the '=>) (make-identifier (car clause) env)))\n                        ",
"            `(,(car (cdr clause)) ,the-key)\n                                    ",
"`(,the-begin ,@(cdr clause)))\n                               ,(loop (cdr clauses",
")))))))))))\n\n  (define-macro parameterize\n    (lambda (form env)\n      (let ((fo",
"rmal (car (cdr form)))\n            (body   (cdr (cdr form))))\n        `(,(the 'w",
...
"                     id)))))\n           (walk (lambda (f form)\n                 ",
"  (cond\n                    ((variable? form)\n                     (f form))\n   ",
"                 ((pair? form)\n                     (cons (walk f (car form)) (w",
"alk f (cdr form))))\n                    ((vector? form)\n                     (li",
"st->vector (walk f (vector->list form))))\n                    (else\n            ",
"         form)))))\n        (list the-quote (walk wrap (cadr form))))))\n\n  (defin",
"e-macro syntax-quasiquote\n    (lambda (form env)\n      (letrec\n          ((wrap ",
...
"-unquote? form)\n          (and (pair? form)\n               (variable? (car form)",
")\n               (variable=? (the 'syntax-unquote) (make-identifier (car form) e",
"nv))))\n\n        (define (syntax-unquote-splicing? form)\n          (and (pair? fo",
"rm)\n               (pair? (car form))\n               (variable? (caar form))\n   ",
"            (variable=? (the 'syntax-unquote-splicing) (make-identifier (caar fo",
"rm) env))))\n\n        (define (qq depth expr)\n          (cond\n           ;; synta",
"x-unquote\n           ((syntax-unquote? expr)\n            (if (= depth 1)\n       ",
"         (car (cdr expr))\n                (list (the 'list)\n                    ",
"  (list (the 'quote) (the 'syntax-unquote))\n                      (qq (- depth 1",
...
"                       (f form))\n                      ((pair? form)\n           ",
"            (cons (walk f (car form)) (walk f (cdr form))))\n                    ",
"  ((vector? form)\n                       (list->vector (walk f (vector->list for",
"m))))\n                      (else\n                       form)))))\n          (le",
"t ((form (cdr form)))\n            (walk unwrap (apply f (walk wrap form))))))))\n",
"\n  (define-macro define-syntax\n    (lambda (form env)\n      (let ((formal (car (",
"cdr form)))\n            (body   (cdr (cdr form))))\n        (if (pair? formal)\n  ",
"          `(,(the 'define-syntax) ,(car formal) (,the-lambda ,(cdr formal) ,@bod",
"y))\n            `(,the-define-macro ,formal (,(the 'transformer) (,the-begin ,@b",
"ody)))))))\n\n  (define-macro letrec-syntax\n    (lambda (form env)\n      (let ((fo",
"rmal (car (cdr form)))\n            (body   (cdr (cdr form))))\n        `(let ()\n ",
